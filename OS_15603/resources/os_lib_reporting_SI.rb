...
module OsLib_Reporting
  ...
  def self.estoloquito(model, sqlFile, runner)
    ...
    if zonashabitablessearch.empty?
      runner.registerError('Did not find value for zonashabitables.')
      # File.open('logpropio.txt', 'a') {|file| file.write("ERROR zonashabitables vacia__\n")}
      # File.open('logpropio.txt', 'a') {|file| file.write("#{zonashabitablesquery}__\n")}
      return false
    else
      # File.open('logpropio.txt', 'a') {|file| file.write("zonashabitables correctas__\n")}
      display = 'Número de zonas habitables'
      source_units = ''
      zonashabitables = zonashabitablessearch.get
    ...
    volumenhabitablequery << "LEFT OUTER JOIN ZoneInfoZoneLists zizl USING (ZoneIndex) "
    volumenhabitablequery << "LEFT OUTER JOIN ZoneLists zl USING (ZoneListIndex) "
    volumenhabitablequery << "WHERE zl.Name != 'CTE_NOHABITA' AND zl.Name != 'CTE_N' "
    volumenhabitablesearch = sqlFile.execAndReturnFirstDouble(volumenhabitablequery)

    if volumenhabitablesearch.empty?
        runner.registerError('No se encontraron zonas habitables para calcular el volumen.')
        # File.open('logpropio.txt', 'a') {|file| file.write("ERROR volumen de zonashabitables vacio__\n")}
        # File.open('logpropio.txt', 'a') {|file| file.write("#{volumenhabitablequery}__\n")}
        return false
    else
      # File.open('logpropio.txt', 'a') {|file| file.write("volumen habitable correcto__\n")}
    ...
    zonasnohabitablesquery = "SELECT * FROM Zones "
    zonasnohabitablesquery << "LEFT OUTER JOIN ZoneInfoZoneLists zizl USING (ZoneIndex) "
    zonasnohabitablesquery << "LEFT OUTER JOIN ZoneLists zl USING (ZoneListIndex) "
    zonasnohabitablesquery << "WHERE zl.Name = 'CTE_NOHABITA' OR zl.Name = 'CTE_N' "
    zonasnohabitablessearch = sqlFile.execAndReturnVectorOfString(zonasnohabitablesquery)

    if zonasnohabitablessearch.empty?  #y los proyectos que no tienen zonas habitables dan error aquí?
        runner.registerError('No se localizaron zonas no habitables.')
        File.open('logpropio.tex', 'a') {|file| file.write("No se localizaron zonas no habitables.\n")}
        File.open('logpropio.tex', 'a') {|file| file.write("zonasnohabitablesquery:\n#{zonasnohabitablesquery}\n")}
        return false
    else
      # File.open('logpropio.txt', 'a') {|file| file.write("zonas no habitables correctas__\n")}
      display = 'Número de zonas no habitables'
      source_units = ''
      value = zonasnohabitablessearch.get.count()
    ...
    superficiescandidatasquery = "SELECT * FROM Surfaces surf "
    superficiescandidatasquery << "INNER JOIN ( "
    superficiescandidatasquery << "#{zonashabitablesquery} "
    superficiescandidatasquery << ") AS zones "
    superficiescandidatasquery << "ON surf.ZoneIndex = zones.ZoneIndex "
    superficiescandidatasquery << "WHERE surf.ClassName <> 'Window' AND surf.ClassName <> 'Internal Mass' "
    superficiescandidatassearch = sqlFile.execAndReturnVectorOfString(superficiescandidatasquery)

    if superficiescandidatassearch.empty?
        runner.registerError('No se localizaron superficies candidatas')
        File.open('logpropio.tex', 'a') {|file| file.write("No se localizaron superficies candidatas.\n")}
        File.open('logpropio.tex', 'a') {|file| file.write("superficiescandidatasquery:\n#{superficiescandidatasquery}\n")}
        return false

    else
      # File.open('logpropio.txt', 'a') {|file| file.write("superficies candidatas correctas__\n")}
      display = 'Número de superficies opacas de zonas habitables'
      source_units = ''
      value = superficiescandidatassearch.get.count()
      medicion_general[:data] << [display, value.to_s, source_units]
      runner.registerValue(display, value, source_units)
    end

    # SUPERFICIES EXTERNAS
    superficiesexternasquery = "SELECT * FROM (#{superficiescandidatasquery}) "
    superficiesexternasquery << "WHERE ExtBoundCond = -1 OR ExtBoundCond = 0 "
    ...
    source_units = ''
    value = superficiesexternassearch.get.count()
    medicion_general[:data] << [display, value.to_s, source_units]
    runner.registerValue(display, value, source_units)
    ...
    areaexterior = areaexteriorsearch.get()
    ...
    superficiesinternasquery = "SELECT * FROM (#{superficiescandidatasquery})
    ...
    superficiesinternas = superficiesinternassearch.get().count()
    # File.open('logpropio.txt', 'a') {|file| file.write("superficies internas correctas__\n")}
    display = 'Número de particiones interiores de las zonas habitables'
    source_units = ''
    medicion_general[:data] << [display, superficiesinternas.to_s, source_units]
    runner.registerValue(display, superficiesinternas, source_units)

    # SUPERFICIES DE CONTACTO
superficiescontactoquery = "SELECT * FROM (#{superficiesinternasquery}) AS internas
...
INNER JOIN (#{zonasnohabitablesquery}) AS znh ON surf.ZoneIndex = znh.ZoneIndex"
    ...
    superficiescontacto = superficiescontactosearch.get().count()
    File.open('logpropio.txt', 'a') {|file| file.write("superficies de contacto correctas__\n")}
    ...
    areainternaquery = "SELECT SUM(GrossArea) FROM (#{superficiescontactoquery})"
    areainternasearch = sqlFile.execAndReturnFirstDouble(areainternaquery)
    areainterna = areainternasearch.get()
    File.open('logpropio.txt', 'a') {|file| file.write("Area interna de la envolvente correcta__\n")}
    display = 'Área de la envolvente térmica que es interior'
    source_units = 'm^2'
    areainterna_neat = OpenStudio.toNeatString(areainterna, 0, true)
    medicion_general[:data] << [display, areainterna_neat, source_units]
    runner.registerValue(display, areainterna, source_units)

    areatotal = areaexterior + areainterna
    File.open('logpropio.txt', 'a') {|file| file.write("Area total correcta__\n")}
