def cte_cambia_u_muros_terreno(model, runner, user_arguments)
  ...
  if u_muros.to_f < 0.001
    runner.registerFinalCondition("No se modifica la transmitancia de los muros (U=0)")
    return true
  end
  ...
  exterior_surface_constructions.each do |exterior_surface_construction|
    ...
    # La resistencia de 0.5 corresponde a una capa de material "terreno" de conductividad (lambda) 2 W/mk de 1 m de espesor
    resistencia_capa = 1 / u_muros - resistencia_termica_sin_aislante - 0.5# siempre que sea positiva, claro
